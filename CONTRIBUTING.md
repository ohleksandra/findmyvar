...
5. **The Release Assets workflow runs automatically** on `release: published`. It builds the plugin and attaches `findmyvar-v<version>.zip` to the release. (Source code archives are auto-generated by GitHub when "Include source code archives" is enabled in repo Settings → General.)
